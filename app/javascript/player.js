;(function (window) {
  ...
  Player.prototype = {
    start: function () {
      ...
      obj.powerButton.addEventListener('click', function (event) {
        obj.power(obj)
      })
    ...
    powerOn: function (obj) {
      ...
      for (var i = 0; i < obj.tracksVolumeSlideres.length; i++) {
        obj.tracksVolumeSlideres[i].addEventListener(
          'change',
          function (e) {
            e.preventDefault()
      ...
      obj.playButton.addEventListener(
        'click',
        function (e) {
          e.preventDefault()
      ...
      obj.rewindButton.addEventListener(
        'click',
        function (e) {
          e.preventDefault()
          obj.rewindTrack(obj)
      ...
      obj.volumeSlider.addEventListener(
        'change',
        function (e) {
    ...
    setTrack: function (track, obj) {
      ...
      var trackVolume = parseInt(
        document.querySelector("[data-trackid='" + info.referenceid + "']").value
      )
    ...
    setElapsedTime: function (obj) {
      ...
      obj.player.addEventListener(
        ...
        function () {
          ...
          songTotalTimeLeft =
            (songMinutesLeft < 10 ? '0' + songMinutesLeft : songMinutesLeft) +
            ':' +
            (songSecondsLeft < 10 ? '0' + songSecondsLeft : songSecondsLeft)
    ...
    setTrackVolume: function (track) {
      ...
      httpRequest.send(
        '_method=patch&track[volume]=' + trackVolume + '&authenticity_token=' + xcsrfToken
      )
